...
4. PR to `main` referencing the issue. CI must be green. Direct pushes to `main` are blocked by a repo-side hook — **always** branch + PR; orchestrator merges with a merge commit (no squash — granular history is kept). **Teardown is part of this step, not an afterthought:** the moment the merge lands, the orchestrator tears down the workspace — `git worktree remove --force <agent-worktree>` then `git branch -d <issue-branch>` (and its `-impl` sibling, plus any `-fixes`/`-fixes-impl` pairs). A worktree/branch outlives its PR only while the PR is open; a merged one is cruft. `scripts/git-cleanup.sh` (run at session start and end) is the safety net that sweeps anything missed — it is not a substitute for tearing down at the merge.
