...
**The structured generation-level decline is the AUTHORITATIVE refusal signal.** The 2026-09-04/05 live release run proved the v2 mechanism — a single threshold on the top reranker score — unsatisfiable on real reranker geometry (a no-answer item scored 0.3885 while answerable conversational items scored 0.0014–0.05; the distributions fully overlap, so no threshold can satisfy both refusal gates), while the model's generation-level honest declines were 10/10 honest end-to-end: grounded decline prose, zero citations, no fabrication. The redesign makes that second line of defense the measured product guarantee:

- **Structured decline contract.** When the supplied passages cannot answer the question at all, the generation system prompt instructs the model to open its response with the machine-readable sentinel `rag.generation.GENERATION_DECLINE_MARKER` on its own first line, followed by the human-readable honest decline (Rule 5's prose, unchanged — it was already good live). The marker is classified at the answer/SSE seam and stripped before display; a marker anywhere after the first line (a quote, or a hostile passage smuggling it in) never classifies as a decline. On the wire a decline is the existing `answer` event, `kind: refusal` — the canned honest-refusal styling applies, and the exchange-log route stays `"retrieval"` with `generation_decline: true` recorded on the validation mapping (no new route or SSE vocabulary).
- **Honesty invariants.** A decline never carries citations, badges, a sources panel or the citation-trust footer, and no factual-sentence validation runs on it. Partial support is unchanged: it is an answer, named per Rule 5, never a decline.
- **Threshold demoted to a conservative pre-filter.** The reranker-score gate survives only as a spend optimisation: below a conservative floor (calibrated as half the minimum answerable calibration score — strictly below every answerable item by construction) retrieval still refuses without a generation call. (This demotion is also why the 2026-09-14 reranker swap to `cross-encoder/ms-marco-MiniLM-L-6-v2` — ADR-006 — is low-risk: the reranker score is a soft, *disabled-in-prod* pre-filter, not the authoritative refusal arbiter, so a small recall change does not move the honesty guarantee. Prod runs with `refusal_threshold=None`, so no recalibration is needed for the swap.) Calibration no longer requires separable distributions: an inseparable-distribution corpus records `separable: false` as a diagnostic and still ships (it no longer bricks the release); a missing or malformed pre-filter artifact degrades to pre-filter-disabled with a warning, never a blocked boot. The gates do not measure the pre-filter alone.
- **Gates measure the authoritative signal.** The §6.2 refusal gate is the fraction of `gate` ∩ `retrieval_refusal` no-answer golds where the authoritative signal fired — pre-filter refusal OR structured generation decline; false-refusal is the authoritative signal firing on answerable golds. The #312 zero-citation heuristic remains as a fallback detector for unmarked declines on no-answer golds; the structured marker takes precedence and, unlike the heuristic, counts on answerable items too. The gold `expected_route` vocabulary is unchanged (`retrieval_refusal` now reads "refused honestly by the retrieval-route pipeline, at either stage"). Refusing without a generation call becomes a measured cost metric, not a correctness gate.
...
- "Show me how CO₂ concentration has risen since 1958" *(chart demo — the wow moment)*
  ...
  live chart starter uses the single-series GISTEMP warming curve, which the
  landed chart pack fully supports. Restore the 10k-year flagship once the data
  is cleared. -->
...
| Reranker | cross-encoder/ms-marco-MiniLM-L-6-v2 (ADR-006; swapped 2026-09-14 from bge-reranker-v2-m3, English-only) | Query-comparable scores for the refusal pre-filter |
