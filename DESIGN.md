...
- **Threshold demoted to a conservative pre-filter.** The reranker-score gate survives only as a spend optimisation: below a conservative floor (calibrated as half the minimum answerable calibration score — strictly below every answerable item by construction) retrieval still refuses without a generation call. Calibration no longer requires separable distributions: an inseparable-distribution corpus records `separable: false` as a diagnostic and still ships (it no longer bricks the release); a missing or malformed pre-filter artifact degrades to pre-filter-disabled with a warning, never a blocked boot. The gates do not measure the pre-filter alone.
