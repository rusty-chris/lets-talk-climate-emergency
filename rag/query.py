...
_PROCESSING_INSTRUCTIONS = (
    ...
    "against the conversation and expands acronyms, and that preserves what "
    "the question asks for — its intent — not only its referents: keep the "
    "user's own wording where it already carries that intent, but where the "
    "question asks what a person can DO about the climate — an action or "
    "personal-agency question ('what can I do', 'what can one person do') — "
    "the standalone query must carry the action vocabulary of the content it "
    "seeks (actions, steps, choices, individual/personal agency, footprint) "
    "rather than collapsing to bare topic nouns, so retrieval reaches the "
    "action evidence and not only the topic; keep the language otherwise "
    "unchanged; (2) classify the message's "
    "scope as exactly one of: in_scope, chart_request, voices, "
    "out_of_scope, adversarial_in_scope, unsafe. Use chart_request only for "
    "an explicit request to plot/chart/graph data. Use voices for "
    "questions seeking the climate movement's own testimony — a voice's own "
    "words, motivations, or story (for example, what a named campaigner says "
    "drives their campaigning), the movement's collective statements (what a "
    "campaign, network, open letter, warning letter or petition said, called "
    "for, or asked of governments), and the get-involved shape (how to get "
    "involved with, join, watch, take part in, or otherwise participate in the "
    "movement) — and never for scientific evidence. A question "
    "asking whether the science, or the evidence, is behind, backs, or "
    "supports a named person's claim is a scientific claim assessment, never "
    "voices: route it as in_scope, or adversarial_in_scope when its framing is "
    "denialist or a gotcha. Classify by what the question asks for, not by "
    "whether a campaigner or activist is named — naming a person is never by "
    "itself enough to make a question voices. "
    "Use adversarial_in_scope for denialist-framed but evidence-answerable "
    "questions. Use out_of_scope for questions the climate-evidence corpus "
    "cannot answer, even when their surface is climate-flavoured: a "
    "climate-adjacent framing (electric cars, aviation emissions, flooding, "
    "carbon footprints) does not by itself make a question answerable from the "
    "evidence. In particular route out_of_scope for consumer purchasing advice, "
    "product comparisons or rankings; market or sales statistics and figures; "
    "company or government spending, budget or finance questions; and questions "
    "about the chatbot or service itself — its own carbon footprint, "
    "operations, or construction. This is a judgement about what the evidence "
    "can support, never about tone: a despairing, doom-phrased or emotional "
    ...
    "question or framed around the climate; that framing does not neutralise "
    "the self-directed signal. This turns on the object of the despair, not "
    "its tone: where the despair instead turns on the state of the climate "
    "evidence, emissions policy, or whether personal action still matters, it "
    "stays in_scope. Always set "
