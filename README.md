...
- **Cross-encoder reranking** (two-stage retrieval) — cross-encoder/ms-marco-MiniLM-L-6-v2 (swapped 2026-09-14 from bge-reranker-v2-m3 for a 14.5× CPU speed-up at held quality; English-only) over the fused candidate set; the reranker's query-comparable scores are load-bearing because they drive the refusal gate, not just precision. Specified in [DESIGN §3.2](DESIGN.md#32-retrieval), [ADR-006](DECISIONS.md#adr-006--hybrid-retrieval-dense--sparse-rrf-with-the-cross-encoder-reranker-in-the-mvp).
- **Selective answering / honest refusal** — the system declines when the supplied evidence cannot answer: a structured generation-level decline is the authoritative, machine-classified refusal signal, with a calibrated retrieval-score pre-filter skipping hopeless queries for $0, and CI-gated refusal and false-refusal rates over the combined signal. Specified in [DESIGN §3.5](DESIGN.md#35-refusal--uncertainty-behaviour-amended-2026-09-05-issue-313-supersedes-the-v2-wording), [ADR-010](DECISIONS.md#adr-010--refusal-when-unsupported-and-calibrated-language-fidelity-as-measured-behaviours).
